...
//The following task involves creating a ProcessQueue class which functions as a priority queue for the Process objects.
public class ProcessQueue {
    ...
    public void addProcess(Process process) {
        ...
        //Now we add the new process and adjust the heap
        pq[++length] = process;
        swim(length);
    }
    ...
    //The following methods that are implemented below are some helper methods I figured could work well and be useful for a minimum binary heap, considering everything we covered during labs.

    // Creates a new array with the specified capacity and copies the existing elements to the new array.
    //The reason I used this helper method is so when the array is full, it doubles its capacity to accommodate more processes.
    ...
    // Here we will check if the priority queue is empty, of course this is crucial so we can determine whether there are any processes left to run
    public boolean isEmpty() {
        return length == 0;
    }

    // Now we use swim up element k to restore the heap order.
    //When a new process is added, it might violate the heap property, so it needs to "swim" up to its correct position in the heap.
    ...
    //And here the opposite process takes place, we restore the heap order by sinking the element down, element at index k
